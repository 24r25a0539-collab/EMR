// ESM test runner directly supported by Node.js 24 without transpiler
async function runFullE2ETests() {
  ...
  try {
    ...
    const patientDbId = profileRes.patient?.id;
    const emrRes = await fetch(`${BASE_URL}/doctors/patients/${patientDbId}/emr`, {
      headers: { Authorization: `Bearer ${docToken}` },
    }).then((r) => r.json());
    ...
    const pendingDoctor = docsRes.doctors?.find((d) => d.verificationStatus === 'PENDING');
    if (pendingDoctor) {
    ...
    console.log('simRes debug:', JSON.stringify(simRes));
    assert(simRes.tampered === true, 'Tampering simulation successfully detected mismatch');
    assert(
      simRes.status === 'INTEGRITY_FAILURE' || simRes.status === 'EMR_INTEGRITY_FAILURE',
      'Tampering status is INTEGRITY_FAILURE'
    );
    ...
    assert(
      auditRes.logs?.some((l) => l.action.includes('EMERGENCY') || l.action.includes('VIEW')),
